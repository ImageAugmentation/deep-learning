...
# root package
sys.path.append(os.path.realpath(os.path.join(os.path.dirname(__file__), "..", "..")))
from pytorch.cli_logger import GANCLILogger
...
class DCGAN(pl.LightningModule):
    def __init__(self, batch_size=32, z_dim: int = 100, nc: int = 3, ngf: int = 64, ndf: int = 64, workers: int = 0,
                 lr=3e-4,
                 b1=0.5, b2=0.9999, datasource: str = 'pytorch', **kwargs):
        super().__init__()
        # Save for checkpoint
    ...
    def training_step(self, batch: list[Tensor], batch_idx: int, optimizer_idx: int):
        ...
        if optimizer_idx == 0:
            ...
            self.cli_logger.log("d_real_loss", loss_discriminator_real, self.global_step, self.current_epoch)
            ...
            self.log("d_fake_loss", loss_discriminator_fake, prog_bar=True, on_epoch=True)
            self.cli_logger.log("d_fake_loss", loss_discriminator_fake, self.global_step, self.current_epoch)
            ...
            self.cli_logger.log("d_loss", loss_discriminator, self.global_step, self.current_epoch)
        ...
        if optimizer_idx == 1:
            ...
            self.cli_logger.log('g_loss', loss_generator, self.global_step, self.current_epoch)
    ...
    def on_epoch_end(self):
        with torch.no_grad():
            ...
            self.logger.experiment.add_image('CelebA Fake Images', grid, self.current_epoch)
            self.cli_logger.fake_images.append(grid)
    ...
    def train_dataloader(self):
        if self.datasource == 'kaggle':
            ...
            celeba_dataset = datasets.CelebA(root="dataset/",
                                             transform=transforms.Compose([
                                                 transforms.Resize((self.ngf, self.ngf)),
                                                 transforms.ToTensor(),
                                                 transforms.Normalize([0.5 for _ in range(self.nc)],
        ...
        #train, _ = random_split(celeba_dataset, [64, len(celeba_dataset) - 64])
        data_loader = DataLoader(celeba_dataset, batch_size=self.batch_size, shuffle=True)
        return data_loader
...
def main(args: Namespace) -> None:
    ...
    model.cli_logger.download_plots()


if __name__ == '__main__':
    parser = ArgumentParser()
    parser.add_argument("--datasource", type=str, default='pytorch', help="Datasource(pytorch|kaggle)")
